...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))  # team-level shared modules

from bedrock_agentcore.runtime import BedrockAgentCoreApp
from memory_agent_base import create_memory_agent
from fallback import FWD2_CONFIG
...
You have MEMORY of earlier ticks — use it to exploit opponent tendencies. At the top of each message you will be given the MANAGER'S ORDERS from your goalkeeper-manager; obey them while playing your role.
...
# --- Wire it up (memory-backed agent + directive-guided handler) ---

agent = create_memory_agent(SYSTEM_PROMPT, MY_PLAYER_ID, POSITION_LABEL, model_id="us.anthropic.claude-haiku-4-5")
create_manager_invoke_handler(app, agent, MY_PLAYER_ID, POSITION_LABEL, FWD2_CONFIG)
