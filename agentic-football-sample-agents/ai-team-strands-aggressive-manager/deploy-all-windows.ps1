<#
.SYNOPSIS
    Deploy all 5 AI Team (Aggressive Manager) agents using the AgentCore CLI (@aws/agentcore)
.DESCRIPTION
    ...
    NOTE (Aggressive Manager team): this team additionally needs AgentCore Memory
    and A2A directive dissemination. This script copies the team-level modules and
    installs the extra deps, but it does NOT set the required environment variables
    (MEMORY_ID, TEAM_RUNTIME_PREFIX, AGENT_RUNTIME_NAME, MANAGER_AUTOSTART) or attach
    the Memory + ListAgentRuntimes/InvokeAgentRuntime IAM policy. The bash
    deploy-all.sh does all of that end-to-end — prefer it (run under WSL on Windows)
    for a fully working deployment, or set the env/IAM manually after this script.
...
Write-Host ""
Write-Host "=========================================="
Write-Host "  AI Team (Aggressive Manager) - Deploy All Agents (New CLI)"
Write-Host "=========================================="
Write-Host ""
...
$projectName = "aiteamaggmgr"
Push-Location (Split-Path $BuildDir -Parent)
$buildDirName = Split-Path $BuildDir -Leaf
$ErrorActionPreference = "Continue"
Write-Host "  Running: agentcore create --name aggmgr_gk_agent --project-name $projectName --output-dir $buildDirName"
& $agentcorePath create --name "aggmgr_gk_agent" --project-name $projectName --build CodeZip --framework Strands --model-provider Bedrock --memory none --skip-git --skip-python-setup --output-dir $buildDirName 2>&1 | ForEach-Object { Write-Host "    $_" }
...
foreach ($agent in $allAgents) {
    $agentNameClean = "aggmgr_" + ($agent -replace 'ai-', '') + "_agent"
    ...
    # Copy team-level shared modules (memory + A2A directive channel) next to src/
    foreach ($mod in @("memory_agent_base.py","directive.py","strategy_channel.py","manager_invoke_handler.py","manager_loop.py")) {
        Copy-Item "$ScriptDir\$mod" "$appDir\$mod" -Force
    }
...
$runtimes = $allAgents | ForEach-Object {
    $name = "aggmgr_" + ($_ -replace 'ai-', '') + "_agent"
    @"
    {
      "name": "$name",
      "build": "CodeZip",
...
if ($deployExitCode -eq 0) {
    ...
    foreach ($agent in $allAgents) {
        $agentNameClean = "aggmgr_" + ($agent -replace 'ai-', '') + "_agent"
        $match = $allRuntimes | Where-Object { $_.name -eq "aiteamaggmgr_$agentNameClean" }
        if ($match) {
